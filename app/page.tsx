...
export default function Home() {
  ...
  const imageFor=(id:string)=>{const d=definitions.find(item=>item.id===id);return d?`/characters/${d.image}`:""};
  ...
  if(guest) return <Frame step="ENTRANCE">
    <section className="entrance-head"><div><p className="eyebrow">WELCOME, {guest.name.toUpperCase()}</p><h1>3人の部隊を編成。</h1></div><div className={`status ${guest.queued?"searching":""}`}>{guest.matchId?"マッチングしました":guest.queued?"対戦相手を検索中":"3枠を選択"}</div></section>
    <section className="party-slots">{Array.from({length:3},(_,index)=>{const id=selected[index];const d=definitions.find(item=>item.id===id);return <button key={index} className={`party-slot ${d?"filled":""}`} disabled={guest.queued||!!guest.matchId} onClick={()=>setEditingSlot(index)}>{d?<><span>SLOT 0{index+1}</span><Image src={`/characters/${d.image}`} alt={d.name} width={360} height={420}/><div><h2>{d.name}</h2><p>HP {d.maxHP} · MOVE COST {d.moveCost}</p></div></>:<><b>＋</b><span>SLOT 0{index+1}</span><p>クリックして選択</p></>}</button>})}</section>
